...
from model.RaBiTMultitask import MultiTask
...
def segment_loss(pred, mask, segment_weight):
    pred_ = pred[segment_weight != 0]
    mask_ = mask[segment_weight != 0]
...
def bi_cls_loss(pred, label):
    # hp classification
    pred_ = torch.flatten(pred[label != -1])
    label_ = label[label != -1].float()
...
class Trainer():
    ...
    def init_model(self):
        ...
        self.net = MultiTask(backbone=encoder, neck=neck)
        ...
        with torch.autograd.set_detect_anomaly(True):
            ...
            for i, data in enumerate(tqdm(self.train_data_loader), start=1):
                ...
                with torch.cuda.amp.autocast(enabled=self.amp):
                    output = self.net(img)
                    seg_loss1 = segment_loss(output["map"][0], mask, segment_weight)
                    seg_loss2 = segment_loss(output["map"][1], mask, segment_weight)
                    seg_loss3 = segment_loss(output["map"][2], mask, segment_weight)
                    seg_loss4 = segment_loss(output["map"][3], mask, segment_weight)
                    seg_loss = seg_loss1 + seg_loss2 + seg_loss3 + seg_loss4

                    hp_loss = bi_cls_loss(output["hp"], hp_label)
                    pos_loss = cls_loss(output["pos"], position_label)
                    type_loss = cls_loss(output["type"], damage_label)

                    loss = seg_loss + hp_loss + pos_loss + type_loss

                epoch_pos_loss.update(pos_loss.item())
                epoch_dmg_loss.update(type_loss.item())
                epoch_hp_loss.update(hp_loss.item())
